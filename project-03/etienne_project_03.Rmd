...
# Convert month number to month name label
weather_tpa <- weather_tpa %>%
  mutate(month_name = month(month, label = TRUE, abbr = FALSE))
  
```

```{r, fig.alt="A faceted histogram showing daily maximum temperatures in Tampa for each month of 2022, with binwidth of 3 degrees Fahrenheit."}
weather_tpa %>%
...
```{r, fig.alt="A single density plot of daily maximum temperatures in Tampa in 2022, showing a right-skewed distribution peaking around 90 degrees Fahrenheit."}
weather_tpa %>%
...
```{r, fig.alt="Twelve density plots faceted by month showing the distribution of daily maximum temperatures in Tampa in 2022."}
weather_tpa %>%
  ggplot(aes(x = max_temp, fill = month_name)) +
...
```{r, fig.alt="A ridgeline plot showing monthly distributions of daily maximum temperatures in Tampa in 2022, colored using the viridis plasma palette."}
weather_tpa %>%
...
```{r, fig.alt="A boxplot showing monthly precipitation levels at Tampa International Airport in 2022, with missing values removed."}
weather_tpa %>%
...
```{r, fig.alt="A horizontal bar chart showing the top 20 most frequent words in Billboard Top 100 lyrics from 2015, colored by frequency using the plasma palette."}
billboard_words %>%
